...
public class ExtendedReport<Datafunctions>{
	...
	{				
		...
		sScreenshot = Datafunctions.getExcelCellData("Screenshot", FunctionalLib.iIteration, FunctionalLib.sheetGlobal);
		
		if(UtilityFunction.waitForElement(sObject, sIdentifier))
		{
	...
	public String takeScreenshot(String FileName) throws Exception
	{
        ...
        try
        {
            ...
            File scrFile = ((TakesScreenshot)UtilityFunction.driver).getScreenshotAs(OutputType.FILE);
	...
	public boolean ReportStepAPIContains(String sTestExpected, String sTestActual, String sReportMessage, int x) throws Exception
	{
		...
		if() data).DataCompareAPI(sTestActual, sTestExpected))
		{
			...
			ReadFromExcel .writeExcel("Status", "Passed_"+ReadFromExcel .getTimeStamp());
		}
		else
		{
			//Report the Fail
			...
			ReadFromExcel .writeExcel("Status", "Failed_"+ReadFromExcel .getTimeStamp());
		}
		System.out.println(sReportMessage+" - "+sReportStatus);
